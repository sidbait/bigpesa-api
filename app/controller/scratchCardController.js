...
module.exports = {
        ...
        userModel.getUserDetails(userToken, async function (err, userDetails) {
            ...
            if (playerId != "") {
                ...
                let query = `select id,prizemaster.prize_title,prizemaster.prize_code,
                                ...
                                prizemaster.prize_image,prizemaster.gratification_type ,prize_details.is_claim,prize_details.winner_date as add_date, 
                                prize_details.credit_date,prize_details.add_date as winner_date,prizemaster.prize_amount
    ...
    checkscratchcard: function (req, res) {
        ...
        userModel.getUserDetails(userToken, async function (err, userDetails) {
            ...
            if (playerId != "") {
                let query = ` select * from fn_checknewscratch_new(${playerId}); `;
                let dbResult = await dbConnection.executeQueryAll(query, 'rmg_db');
                if (dbResult != null && dbResult != undefined && dbResult.length > 0) {
